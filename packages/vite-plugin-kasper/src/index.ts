...
interface KasperBlock {
  template: string;
  script: string;
  style: string;
}

function parseBlocks(source: string): KasperBlock {
...
function extractClassName(script: string): string | null {
...
export default function kasper(): Plugin {
  return {
    name: "vite-plugin-kasper",
    async transform(source, id) {
      if (!id.endsWith(".kasper")) return;
