...
<CardGrid>
  ...
  <Card title="Template Directives" icon="list-format">
    `@if`, `@each`, `@let`, `@on`, `@attr`, `@ref` — valid HTML attributes with surgical DOM updates and automatic cleanup.
  </Card>
  <Card title="AI Agent Ready" icon="star">
    16KB, zero dependencies, entire API fits in a single context window. Paste [llms.txt](https://kasperjs.top/llms.txt) into any agent and it can write idiomatic Kasper code from scratch.
  </Card>
